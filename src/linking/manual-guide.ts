...
export class ManualLinkingGuideGenerator {
   ...
   */
  public generateGuide(options: ManualGuideOptions): string {
   ...
   */
  private generateIntroduction(options: ManualGuideOptions): GuideSection {
    const platformText = options.platform === 'both' ? 'Android and iOS' : 
                        options.platform === 'android' ? 'Android' : 'iOS';
...
`;
   ...
   */
  private generatePrerequisites(options: ManualGuideOptions): GuideSection {
    const content = `
...
`;
   ...
   */
  private generateAndroidSections(options: ManualGuideOptions): GuideSection[] {
    const sections: GuideSection[] = [];
...
`.trim();
   ...
   */
  private generateAndroidBuildGuide(options: ManualGuideOptions): string {
...
`.trim();
   ...
   */
  private generateIOSSections(options: ManualGuideOptions): GuideSection[] {
    const sections: GuideSection[] = [];
   ...
   */
  private generateIOSPodfileGuide(options: ManualGuideOptions): string {
...
`.trim();
   ...
   */
  private generateVerificationSection(options: ManualGuideOptions): GuideSection {
    const content = `
...
`;
   ...
   */
  private generateTroubleshootingSection(options: ManualGuideOptions): GuideSection {
    const content = `
...
`;
   ...
   */
  private formatGuide(sections: GuideSection[], format: 'markdown' | 'text'): string {
    if (format === 'markdown') {
   ...
   */
  private formatAsMarkdown(sections: GuideSection[]): string {
    let output = '';
   ...
   */
  private formatAsText(sections: GuideSection[]): string {
    let output = '';
   ...
   */
  public saveGuide(guide: string, outputPath: string): void {
  ...
  public generateProjectGuide(
    ...
    const options: ManualGuideOptions = {
      platform,
      projectRoot,
      moduleConfig,
 ...
 */
export function generateManualGuide(
  projectRoot: string,
  platform: 'android' | 'ios' | 'both' = 'both',
  outputPath?: string
): string {

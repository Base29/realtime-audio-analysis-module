 ...
 */
const SpectrumVisualizer = ({ frequencyData, barCount, isAnalyzing, }) => {
    // Animated values for each bar height
    const barHeights = useRef([]);
    // Peak hold values and animated values
    const peakHoldValues = useRef([]);
    const peakHoldAnimatedValues = useRef([]);
    const peakHoldDecayTimeouts = useRef([]);
    ...
    useEffect(() => {
        ...
        frequencyHistoryRef.current = new RingBuffer(10, []);
    ...
    const getBarColor = (index, amplitude) => {
    ...
    const processFrequencyData = (data, targetBarCount) => {
        if (!data.length)
    ...
    const updatePeakHold = (index, currentAmplitude) => {
    ...
    const renderBar = (index) => {
        ...
        return (React.createElement(View, { key: index, style: [styles.barContainer, { width: barDimensions.width }] },
    ...
    return (React.createElement(View, { style: styles.container },
        React.createElement(View, { style: [styles.barsContainer, { height: barDimensions.maxHeight }] }, Array.from({ length: barCount }, (_, index) => renderBar(index)))));
};

...
import { AudioLevelsHook, AnalysisConfig, AudioAnalysisEvent, PermissionStatus } from '../types/interfaces';
 ...
 */
export function useRealtimeAudioLevels(): AudioLevelsHook {
  // Permission state
  ...
  const persistedConfigRef = useRef<AnalysisConfig>({
    fftSize: 1024,
    sampleRate: 44100,
    smoothing: 0,
  });
  ...
  const handleAudioData = useCallback((event: AudioAnalysisEvent) => {
  ...
  const startAnalysis = useCallback(async (config: AnalysisConfig = {}): Promise<void> => {
    try {
      setError(null);
  ...
  const setSmoothing = useCallback(async (enabled: boolean, factor: number): Promise<void> => {
    try {
  ...
  const setFftConfig = useCallback(async (newFftSize: number, downsampleBins: number): Promise<void> => {
    try {

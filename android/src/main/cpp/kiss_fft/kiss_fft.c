...
/* The guts header contains all the multiplication and addition macros that are
 defined for fixed or floating point complex numbers.  It also contains the main
                    ...
                    const kiss_fft_cfg st)
{
  ...
  const int m = *factors++; /* stage's fft nfft/p */
...
#pragma omp parallel for
  ...
  else
  {
    do
    {
      ...
      // each one with a different driver
                     ...
                     kiss_fft_cpx *fout, int in_stride)
{
  if (fin == fout)
  {
    ...
    kiss_fft_cpx *tmpbuf =
        (kiss_fft_cpx *)KISS_FFT_MALLOC(sizeof(kiss_fft_cpx) * st->nfft);
...
{
  // nothing needed for standard alloc
}
...
int kiss_fft_next_fast_size(int n)
{
  while (1)
  {
    ...
    while ((m % 5) == 0)
      m /= 5;
    if (m <= 1)
      break; /* n is completely factorable by primes up to 5 */
    n++;
  }
  return n;
}
